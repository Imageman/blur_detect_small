...
# write_json json to txt file
def write_json(dic, path):
    with open(path, 'w+') as f:
        f.write(json.dumps(dic))


# read_json from txt file and transfer to json
def read_json(path):
...
def copy_file(target_patch, name):
    if not os.path.isdir(target_patch):
    ...
    shutil.copy(name, os.path.join(target_patch, name))
...
def read_single_image_to_tensor(path, mean=None, std=None):
    # image.shape=(h, w, c)
...
def dir_filename_join(prefix, path):
